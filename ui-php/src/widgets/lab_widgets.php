    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        ?>
          ...
          <p><strong>Lab:</strong> <?= htmlspecialchars($active_lab['lab_title'] ?? 'Docker & Kubernetes Basics') ?></p>
          <p><strong>Expires:</strong> <?= date('d M Y, h:i A', strtotime($active_lab['access_expiry'])) ?></p>
          <a href="terminal.php" class="btn btn-primary btn-block">
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        $stmt = $conn->prepare("
            SELECT 
                COUNT(*) as total_labs,
                SUM(CASE WHEN status='completed' THEN 1 ELSE 0 END) as completed_labs,
        ...
        $completed = $progress['completed_labs'] ?? 0;
        $avg = round($progress['avg_progress'] ?? 0);
        ?>
        
        <div class="row text-center">
          <div class="col-6">
            <h4><?= $completed ?>/<?= $total ?></h4>
            <small class="text-muted">Labs Completed</small>
          </div>
          <div class="col-6">
            <h4><?= $avg ?>%</h4>
        ...
        <div class="progress mt-3" style="height: 25px;">
          <div class="progress-bar" role="progressbar" 
               style="width: <?= $avg ?>%; background: linear-gradient(135deg, #667eea, #764ba2);">
            <?= $avg ?>%
          </div>
        </div>
